...
def print_results(subj_nbr, results_dict, mode="test", return_train_score=False):
    ...
    """
    print(
        f"{'='*10} Classification Scores Comparison for Patient {subj_nbr} {mode} "
        f"mode) {'='*10}"
    )
    scores_results_dict = {}
    methods = []
    ...
    for pipeline, scores_value in results_dict.items():
        ...
        for score_name, value in scores_value.items():
            ...
            scores_results_dict[score_name][pipeline] = [mean_score, np.std(value)]
            cnt_loop2 += 1
        cnt_loop1 += 1
                ...
                for pipeline, scores_value in result_dict_subj.items():
                    ...
                    for score_name, value in scores_value.items():
                        ...
                        if subj == len(results_list):
                            dict_all["average"][pipeline][score_name] = round(
                                dict_all["average"][pipeline][score_name] / subj, 3
                            )
    ...
    for score_name, pipelines in scores_results_dict.items():
        ...
        rects_list.append(
            ax.bar(
                ...
                label=f"{score_name}",
                edgecolor="white",
                capsize=8 * bar_width / len(scores_results_dict),
                alpha=0.5,
            )
        )
        counter += 1
